      ...
      <header className="h-16 bg-white border-b flex items-center justify-between px-4 sm:px-6 sticky top-0 z-50 no-print">
        ...
        <div className="flex items-center gap-1 sm:gap-3">
          ...
          {user ? (
            ...
            <Button variant="ghost" size="sm" onClick={() => { setAuthMode("login"); setShowAuthModal(true); }} className="text-blue-600 font-bold hover:bg-blue-50">
              <LogIn className="w-4 h-4 mr-1 sm:mr-2" /> <span className="hidden sm:inline">Login</span>
            </Button>
          )}
        </div>
      </header>
        ...
        <AnimatePresence>
          {showAuthModal && (
            <motion.div 
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              className="fixed inset-0 z-[70] bg-black/40 backdrop-blur-sm flex justify-center items-center p-4 overflow-y-auto"
                ...
                <div className="p-8">
                  <header className="text-center space-y-2 mb-8">
                    ...
                    <p className="text-slate-500 text-sm">
                      {authMode === "login" ? "Login to sync your invoices across devices" : "Sign up to track and manage your history"}
                    </p>
                  </header>

                  <form onSubmit={async (e) => {
                    await handleAuth(e);
                    if (auth.currentUser) setShowAuthModal(false);
                  }} className="space-y-4">
                    {authMode === "signup" && (
                    ...
                    <Button type="submit" disabled={isAuthLoading} className="w-full bg-blue-600 hover:bg-blue-700 h-11 font-bold">
                  ...
                  <div className="mt-4">
                    ...
                    <Button 
                      variant="outline" 
                      type="button" 
                      className="w-full mt-4 h-11 gap-2 border-slate-200 font-bold" 
                      onClick={async () => {
